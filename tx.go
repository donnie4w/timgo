...
// TIMTYPE tim 接口类型 type of interface
type TIMTYPE byte
...
// 使用用户名密码域名注册
func (ts *tx) register(username, pwd, domain string) []byte {
...
// 使用用户名密码域名获取token
func (ts *tx) token(username, pwd, domain string) []byte {
	ta := &TimAuth{Name: &username, Pwd: &pwd, Domain: &domain}
	buf := NewBuffer()
...
// 使用用户名密码域名登录
func (ts *tx) loginByAccount(username, pwd, domain, resource string, termtyp int8, extend map[string]string) []byte {
	ts.ta.Name, ts.ta.Pwd, ts.ta.Domain, ts.ta.Resource, ts.ta.Termtyp, ts.ta.Extend = &username, &pwd, &domain, &resource, &termtyp, extend
	buf := NewBuffer()
...
func (ts *tx) _message(timtype TIMTYPE, mstype, odType int8, msg string, to string, roomId string, udshow int16, udtype int16, msgId int64, dataBinary []byte, extend map[string]string, extra map[string][]byte) []byte {
...
func (ts *tx) message2Friend(msg string, to string, udshow int16, udtype int16, extend map[string]string, extra map[string][]byte) []byte {
	return ts._message(TIMMESSAGE, 2, 1, msg, to, "", udshow, udtype, 0, nil, extend, extra)
}

func (ts *tx) messageByPrivacy(msg string, to string, roomId string, udshow int16, udtype int16, extend map[string]string, extra map[string][]byte) []byte {
	return ts._message(TIMMESSAGE, 2, 1, msg, to, roomId, udshow, udtype, 0, nil, extend, extra)
}

func (ts *tx) message2Room(msg string, roomId string, udshow int16, udtype int16, extend map[string]string, extra map[string][]byte) []byte {
	return ts._message(TIMMESSAGE, 3, 1, msg, "", roomId, udshow, udtype, 0, nil, extend, extra)
}

func (ts *tx) revokeMessage(msgId int64, to, room string, msg string, udshow int16, udtype int16) []byte {
	return ts._message(TIMREVOKEMESSAGE, 2, 2, msg, to, room, udshow, udtype, msgId, nil, nil, nil)
}

func (ts *tx) burnMessage(msgId int64, msg string, to string, udshow int16, udtype int16) []byte {
	return ts._message(TIMBURNMESSAGE, 2, 3, msg, to, "", udshow, udtype, msgId, nil, nil, nil)
}

func (ts *tx) stream(msg []byte, to string, room string, udShow, udType int16) []byte {
...
// func (ts *tx) exMessage(odType int8, msg string, to string, roomId string, udshow int16, udtype int16, extend map[string]string, extra map[string][]byte) (_r []byte) {
// 	if odType > 30 {
// 		_r = ts._message(TIMMESSAGE, 2, odType, msg, to, roomId, udshow, udtype, 0, extend, extra)
// 	} else {
...
func (ts *tx) _presence(timtype TIMTYPE, to string, show int16, status string, toList []string, subStatus int8, extend map[string]string, extra map[string][]byte) []byte {
...
func (ts *tx) presence(to string, show int16, status string, subStatus int8, extend map[string]string, extra map[string][]byte) []byte {
	return ts._presence(TIMPRESENCE, to, show, status, nil, subStatus, extend, extra)
}

func (ts *tx) presenceList(show int16, status string, subStatus int8, extend map[string]string, extra map[string][]byte, toList []string) []byte {
	return ts._presence(TIMPRESENCE, "", show, status, toList, subStatus, extend, extra)
}

func (ts *tx) broadPresence(subStatus int8, show int16, status string) []byte {
	return ts._presence(TIMBROADPRESENCE, "", show, status, nil, subStatus, nil, nil)
}

func (ts *tx) pullmsg(rtype int32, to string, mid, limit int64) []byte {
...
func (ts *tx) offlinemsg() []byte {
	buf := NewBuffer()
	buf.WriteByte(byte(TIMOFFLINEMSG))
	return buf.Bytes()
}

func (ts *tx) addroster(node string, msg string) (_r []byte) {
	buf := NewBuffer()
...
func (ts *tx) rmroster(node string) (_r []byte) {
	buf := NewBuffer()
...
func (ts *tx) blockroster(node string) (_r []byte) {
	buf := NewBuffer()
...
func (ts *tx) modifyPwd(oldpwd, newpwd string) (_r []byte) {
	buf := NewBuffer()
...
func (ts *tx) roomusers(node string) []byte {
	buf := NewBuffer()
...
func (ts *tx) newroom(gtype int64, roomname string) []byte {
	buf := NewBuffer()
...
func (ts *tx) addroom(gnode string, msg string) []byte {
	buf := NewBuffer()
...
func (ts *tx) pullroom(rnode, unode string) []byte {
	buf := NewBuffer()
...
func (ts *tx) nopassroom(rnode, unode, msg string) []byte {
	buf := NewBuffer()
...
func (ts *tx) kickroom(rnode, unode string) []byte {
	buf := NewBuffer()
...
func (ts *tx) leaveroom(gnode string) []byte {
	buf := NewBuffer()
...
func (ts *tx) cancelroom(gnode string) []byte {
	buf := NewBuffer()
...
func (ts *tx) blockroom(gnode string) []byte {
	buf := NewBuffer()
...
func (ts *tx) blockroomMember(gnode, tonode string) []byte {
	buf := NewBuffer()
...
func (ts *tx) blockrosterlist() []byte {
	buf := NewBuffer()
...
func (ts *tx) blockroomMemberlist(gnode string) []byte {
	buf := NewBuffer()
...
func (ts *tx) virtualroom(rtype int32, vnode, unode string, i int64) []byte {
	buf := NewBuffer()
...
func (ts *tx) pushstream(node string, body []byte, dtype int8) []byte {
	t := &TimStream{VNode: node, Body: body}
	if dtype != 0 {
		t.Dtype = &dtype
	}
	buf := NewBuffer()
...
func (ts *tx) nodeinfo(ntype int32, nodelist []string, usermap map[string]*TimUserBean, roommap map[string]*TimRoomBean) []byte {
...
func (ts *tx) bigString(node string, datastring string) []byte {
	buf := NewBuffer()
...
func (ts *tx) bigBinary(node string, dataBinary []byte) []byte {
	buf := NewBuffer()
