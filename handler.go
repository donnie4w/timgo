...
func (tc *TimClient) LoginByToken(username, token string, domain *string, resource string, termtyp int8, extend map[string]string) (err error) {
	tc.closeconnect()
	tc.ts.loginByToken(username, token, domain, resource, termtyp, extend)
	return tc.connect()
}
