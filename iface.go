...
var logger = logging.NewLogger().SetFormat(logging.FORMAT_LEVELFLAG | logging.FORMAT_DATE | logging.FORMAT_TIME)
...
func (tc *TimClient) LoginByToken(token int64, resource string, termtyp int8, extend map[string]string) (err error) {
	tc.closeconnect()
